...
use spin::Mutex;
use user_lib::{
    claim_ext_int, get_time, init_user_trap, set_ext_int_enable, set_timer, sleep, user_uart::*,
    yield_,
};
...
lazy_static! {
    pub static ref INTR_SERIAL: Arc<Mutex<BufferedSerial>> = Arc::new(Mutex::new(
        BufferedSerial::new(get_base_addr_from_irq(UART_IRQN.load(Relaxed)))
    ));
}
...
lazy_static! {
    pub static ref POLLING_SERIAL: Arc<Mutex<PollingSerial>> = Arc::new(Mutex::new(
        PollingSerial::new(get_base_addr_from_irq(UART_IRQN.load(Relaxed)))
    ));
}
...
fn user_polling_test() -> (usize, usize, usize) {
    ...
    POLLING_SERIAL.lock().hardware_init(3_125_000);
...
fn user_intr_test() -> (usize, usize, usize) {
    let uart_irqn = UART_IRQN.load(Relaxed);
    let claim_res = claim_ext_int(uart_irqn as usize);
    INTR_SERIAL.lock().hardware_init(3_125_000);
